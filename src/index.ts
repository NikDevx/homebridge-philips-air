    ...
    async updateTemperature(purifier: Purifier): Promise<void> {
        ...
        try {
            ...
            if (purifier.config.temperature_sensor) {
                ...
                let counter = purifier.config.polling_interval_humidity_sensor;
                setInterval(function () {
                    counter--
                    if (counter === 0) {
                        temperature_sensor.updateCharacteristic(hap.Characteristic.CurrentTemperature, status.temp);
                    }
                }, purifier.config.polling_interval_humidity_sensor * 1000);
            }
        } catch (err) {
            ...
            try {
                ...
                if (purifierService) {
                    ...
                    purifierService
                        ...
                        .updateCharacteristic(hap.Characteristic.TargetAirPurifierState, 0)
                ...
                if (purifier.config.humidifier) {
                    ...
                    if (status.pwr == '1') {
                        ...
                        // if (buttonsService) {
                        // buttonsService.updateCharacteristic(hap.Characteristic.On, true);
                        // }
                        if (Humidifier) {
                            Humidifier
                                .updateCharacteristic(hap.Characteristic.Active, status.pwr)
                                ...
                                // .updateCharacteristic(hap.Characteristic.HumidifierThreshold, 40)
        ...
        if (purifier) {
            ...
            if (speed > 0) {
                ...
                try {
                    ...
                    if (service) {
                        service
                            .updateCharacteristic(hap.Characteristic.TargetRelativeHumidity, values.rhset)
                            .updateCharacteristic(hap.Characteristic.CurrentHumidifierDehumidifierState, 2)
                            .updateCharacteristic(hap.Characteristic.TargetHumidifierDehumidifierState, 1);
                    ...
                    purifier.timeout = setTimeout(() => {
                        if (service) {
                            service
                                .updateCharacteristic(hap.Characteristic.RotationSpeed, speed_humidity);
                                // .updateCharacteristic(hap.Characteristic.RelativeHumidityHumidifierThreshold, values.rhset);
                        }
        ...
        if (!accessory) {
            ...
            if (config.light_control) {
                accessory.addService(hap.Service.Lightbulb, 'Lights', 'Lights')
                    // .addCharacteristic(hap.Characteristic.Lightbulb)
            ...
            let WaterLevel = accessory.getService('Water level');
            if (config.water_level) {
                ...
                if (WaterLevel != undefined) {
                    accessory.removeService('Water level');
                }
            }
            let temperature_sensor = accessory.getService('Temperature');
            if (config.temperature_sensor) {
            ...
            let humidity_sensor = accessory.getService('Humidity');
            if (config.humidity_sensor) {
                ...
                if (humidity_sensor != undefined) {
                    accessory.removeService('Humidity');
                }
            }
            let Humidifier = accessory.getService('Humidifier');
            if (config.humidifier) {
